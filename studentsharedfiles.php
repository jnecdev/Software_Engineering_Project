					...
					<div>
						<div class="section">
							...
							<?php
								...
								$student = $number;
								$token = "";
								$usertable = "$subject"."_"."$student"."_files";
								if(isset($_GET['id']))
									{
										$id=intval($_GET['id']);
										$query = "SELECT title, type, size, file FROM $usertable WHERE id=$id";
									...
									if(mysql_num_rows($result) == 0)
									{
										...
										Print "<li><p>";
?>										
											<a href="studentsharedfiles.php?id=<?php echo $id ?>&token=<?php echo""?>&fname=<?php echo $fname?>&lname=<?php echo $lname?>&mname=<?php echo $mname?>&number=<?php echo $number?>"><?php echo $title ?></a>
