			...
			<div>
				...
				<div>
					...
					<div class="register">
						...
						<form action="editfacultyinfo.php" method="POST">
							...
							<div>
								...
								if($_SERVER['REQUEST_METHOD'] == 'POST') {
									...
									if(empty($password)){
										...
										$user_input = mysql_query("UPDATE $user2 SET section='$section', address='$address', zip_code='$zip', phone_number='$pnumber', cell_number='$cnumber', fname='$fname', mname='$mname', lname='$lname', email='$email'");

										$teach = "$fname $mname, $lname";

										$temp = "$user"."_sbsn";

										$temp2 = mysql_query("Select * from $temp");

										while($row = mysql_fetch_assoc($temp2)){
												$subject = $row['subject'];
										}

										mysql_query("UPDATE subject_list SET teacher='$teach' WHERE code='$subject'");
									...
									$query = mysql_query("Select password from faculty WHERE username='$user'");
...
?>
									...
									<tr>
										...
										<td><a href="adminportalindex.php"><label for="name"><span>B</span>ack to Profile</label></td>
